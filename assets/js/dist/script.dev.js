...
function movieCase(poster, name, year, genre) {
  ...
  film += "<img class=\"card-img-top\" src=\"https://image.tmdb.org/t/p/w500".concat(poster, "\" alt=\"CardImageCap\">");
  film += "<h3 class=\"card-title\">".concat(name, "</h3>");
...
fetch('https://api.themoviedb.org/3/discover/movie?api_key=a05fba96f4d3bad807d07845d4896afb&language=en-US&sort_by=popularity.desc&include_adult=false&include_video=false&page=1&with_crew=608&with_companies=10342').then(function (response) {
  ...
  exampleCards.forEach(function (x) {
    ...
    document.getElementById("cardList").innerHTML += "<li>".concat(movieCase(x.poster, x.name, x.year, x.genre_name), "</li>");
  });
});